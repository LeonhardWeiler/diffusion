...
android {
    ...
    lint {
        ...
        // JGit and jsch are ordinary jvm libraries, and they reference corners
        // of the jdk that Android does not have — jsch has a jcraft agent proxy
        // behind a class check, JGit an api for the system git config it never
        // reaches here. Lint cannot see that those branches are dead, and this
        // build fails on a lint error.
        disable += "InvalidPackage"
    }

    packaging {
        resources {
            // Three jars carry a license and a service index under META-INF,
            // and packaging refuses to pick between them.
            excludes += setOf(
...
dependencies {
    ...
    // Git. JGit brings the repository, the ssh transport is a module of its own
    // — and the jsch it asks for is the abandoned com.jcraft one, so it is
    // swapped for the fork that still gets releases and knows ed25519.
    implementation(libs.jgit)
    implementation(libs.jgit.ssh.jsch) {
        exclude(group = "com.jcraft", module = "jsch")
    }
    implementation(libs.jsch)
    // ed25519: neither Android nor jsch bring it, and it is what a key
    // generated here is made of.
    implementation(libs.bouncycastle)
    // JGit logs through slf4j and says so loudly when nothing is bound
